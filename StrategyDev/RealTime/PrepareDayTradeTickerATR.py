...
from PrepareDayTradeTicker import writeStrongTicker
import os
parent = os.path.dirname(os.path.abspath(__file__))
StrongPath = os.path.join(parent, 'StrongTickersATR')
...
def main(td = datetime.today(), min_price=0, max_price=50, num_shares=10000, shares_ratio=0):
    try:
        ...
        data = list(table.find({'Date':{'$gte':td.strftime('%Y-%m-%d'), '$lte':td.strftime('%Y-%m-%d')}, "Ticker" :{"$in":list(info_data.keys())}}))
        ...
        # # 強勢股
        # last_df = last_df[last_df.Volume >= 3000]
        # last_df['Ret'] = 0
        # last_df["HighRet"] = 0
        ...
        for ticker in ticker_volume_filted:#df.Ticker.unique():
            try:
                ...
                temp_df = Calc.MA(temp_df, [5, 10, 20], 'Close')
                ...
                temp_df['HL'] = temp_df["High"] - temp_df['Low']
                temp_df['HC'] = (temp_df["High"] - temp_df['Close'].shift(1)).apply(lambda x: abs(x))
                temp_df['LC'] = (temp_df["Low"] - temp_df['Close'].shift(1)).apply(lambda x: abs(x))
                temp_df['ATR'] = temp_df[['HL',"HC","LC"]].max(axis=1).rolling(5).mean()
                temp_df['ATR%'] = temp_df['ATR'] / temp_df['Close'].shift(1)
                ...
                condi_1 = temp_df.VMA5.iloc[-1] >= 3000
                condi_2 = temp_df.slope_10.iloc[-1] > 0 and temp_df.slope_5.iloc[-1] > 0 and temp_df.slope_20.iloc[-1] > 0
                condi_3 = temp_df.MA5.iloc[-1] > temp_df.MA10.iloc[-1]
                condi_4 = temp_df.MA10.iloc[-1] > temp_df.MA20.iloc[-1]
                condi_5 = temp_df['ATR%'].iloc[-1] >= 0.05
                ...
                if all([condi_1, condi_2, condi_3, condi_4, condi_5]): #, condi_8, condi_6, condi_7
                    up_limit_ticker[ticker] = {
                        'PreClose':temp_df.Close.iloc[-1],
                        # 'PreHigh':temp_df.High.iloc[-1],
                        # 'PreLow':temp_df.Low.iloc[-1]
                    }
        ...
        writeStrongTicker(next_trade_day.strftime('%Y-%m-%d'),dict(up_limit_ticker), StrongPath)
        # expand_text = "隔日沖，收盤價買入，隔天2%停利，5%停損，皆未觸及則收盤價出"
...
if __name__ == '__main__':
    ...
    main(datetime(2022,2,23),min_price=0, max_price=9999, num_shares=1000, shares_ratio=1.5)
