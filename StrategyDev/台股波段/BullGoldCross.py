...
highProbTable = pd.read_csv(os.path.join(parent, "無腦交易","Output","Bull_Trend_With_MACD_Cross","Summary_prob_holding_tradenum.csv"), encoding='utf-8-sig')
bt_sum_Table = pd.read_csv(os.path.join(parent, "無腦交易","Output","Bull_Trend_With_MACD_Cross","Summary.csv"), encoding='utf-8-sig')
    ...
    try:
        ...
        for ticker in info_data.keys():#df.Ticker.unique():
            ...
            try:
                ...
                long_trend_condi = temp_df.MA5.iloc[-1] > temp_df.MA10.iloc[-1] and temp_df.MA10.iloc[-1] > temp_df.MA20.iloc[-1] and temp_df.MA20.iloc[-1] > temp_df.MA60.iloc[-1]
                g_condi = temp_df.DIF.iloc[-1] >= temp_df.MACD.iloc[-1] and all(temp_df['DIF'].loc[temp_df.index[-5]:temp_df.index[-1]] < temp_df['MACD'].loc[temp_df.index[-5]:temp_df.index[-1]])
                ...
                if long_trend_condi and g_condi:# and bt_sum_Table[bt_sum_Table.代號==ticker]['總交易次數'].values[0] >= 10 and bt_sum_Table[bt_sum_Table.代號==ticker]['勝率%'].values[0] > 50:
                    try:
        ...
        description = '無腦秘書3號，多頭排列，MACD黃金交叉>=做多\n'
        ...
        saveRecommand(final_select, 'BullGoldCross')
        ...
        fig_path = CreateRecommendFig(td, final_select, bt_Prob=bt_Prob, holding_p=holding_p, highProb=highProb, 
                                        ...
                                        algo_num=3, description=description, entry_method=entry_method, take_profit_rate=10, 
                                        stop_loss_rate=10, algo_name='BullGoldCross')
