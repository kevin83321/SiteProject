...
def sendPhoto(text, fig_full_path, useTele=True):
    if useTele:
        tele.sendPhoto(text, fig_full_path)
    else:
        Line.sendMessage(text, img=fig_full_path, TokenKey='NoBrain')

def sendMessage(text, useTele=True):
    if useTele:
        tele.sendMessage(text)
    else:
        Line.sendMessage(text,TokenKey='NoBrain')
...
def createRecommandTable(tickers, bt_Prob=None, holding_p=None, highProb=None):
    try:
        table = PrettyTable()
        table.align = 'r'
        if bt_Prob:
            ...
            table.add_column('btProb', bt_prob)
            table.add_column('HoldAvg', h_p)
            table.add_column('Suggest', h_prob)
            return table
        else:
            table.add_column('Stock', tickers)
...
def sendResultTable(td, tickers, momentum=None, bt_Prob=None, holding_p=None, highProb=None, 
                    algo_num=1, expand_text='', useTele=True):
    try:
        table = createRecommandTable(tickers, momentum,bt_Prob, holding_p, highProb)
        tdStr = td.strftime('%Y-%m-%d')
        text = f'\n{tdStr} \n'
        text += f'策略{algo_num}號\n'
        text += '下一交易日開盤價可以考慮的標的\n'
        text += 'Suggest = 1則是持倉周期較短，勝率較高的\n\n'
        text += table.get_string()
        text += '\n\n' + expand_text
        sendMessage(text, useTele)
    except:
        print(GetException())
