...
def search_file(service, update_drive_service_name, is_delete_search_file=False):
    ...
    #https://drive.google.com/drive/u/0/folders/18mn9MeBVf8Hrq6dX7tUfY-3p4XarqFIB
    items = results.get('files', [])
    #print(dir(service.files().list()))
    print(dir(service.drives().list()))
    if not items:
        ...
        for item in items:
            ...
            for child in children.get('files',[]):
                if child['mimeType'] == 'application/vnd.google-apps.folder' and child['name'] == '啟芳':
                    print(child)
                    folderId=child['id']
                    grands = service.files().list(
                        q=f"'{folderId}' in parents"
                        ).execute()
                    for grandchild in grands.get('files',[]):
                        print(grandchild)
            #print(children)
            #for child in children.get('items', []):
            #for child in children.get('files',[]):
                ...
                #print(f'File Name: {child["name"]}, File Id: {child["id"]}' )
            #print(dir(service.files()))
            #for i in service.ListFile({'q':'trashed=false'}).GetList():
            #   print(i)
            ...
            if times == len(items): return item['id']
            times += 1
...
if __name__ == '__main__':
    ...
    if not creds or creds.invalid:
        ...
        creds = tools.run_flow(flow, store)
    service = build('drive', 'v3', http=creds.authorize(Http()))
    search_file(service, update_drive_service_name='外包')
    #folder_id_list = search_folder(service, update_drive_folder_name='外包')
    #print_files_in_folder(service, folder_id_list[0])