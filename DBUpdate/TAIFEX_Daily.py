...
def Options_Data_Daily(lastDate, opt_table=None):
    td = datetime.now()
    start_date = datetime.strptime(lastDate, '%Y-%m-%d')
    end_date = min(start_date + timedelta(days=30), td)
    datas = []
...
if __name__ == '__main__':
    ...
    df = Options_Data_Daily(opt_lastDate, opt_table)
    if not df.empty:
        parallel_update(opt_table, createTickerOpt, df)

    Tele().sendMessage(f'盤後爬取期交所,所有期貨/選擇權收盤價成功', group='UpdateMessage')