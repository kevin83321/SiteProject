...
__updated__ = '2022-09-16 15:41:18'
...
def main(date=datetime.today()):
    ...
    Tele().sendMessage(f"現在開始{date.strftime('%Y-%m-%d')}抓取漲停股", group='UpdateMessage')
    ...
    Tele().sendMessage(f"上市漲停股 共: {len_tse} 隻", group='UpdateMessage')
    Tele().sendMessage(f"上櫃漲停股 共: {len_otc} 隻", group='UpdateMessage')
    ...
    api.logout()
    Tele().sendMessage(f"今日({date.strftime('%Y-%m-%d')})漲停股 共: {len_tse + len_otc} 隻", group='UpdateMessage')
    os._exit(0)
