    ...
    try:
        ...
        for ticker in final_tickers:#df.Ticker.unique():
            try:
                ...
                bar_last_condi = temp_df.High.iloc[-1] > temp_df.High.iloc[-2] and temp_df.Low.iloc[-1] > max(temp_df.MA5.iloc[-1], temp_df.Low.iloc[-2])
                bar_last2_condi = temp_df.High.iloc[-1] > max(temp_df.Close.iloc[-1], temp_df.Open.iloc[-1]) and temp_df.Low.iloc[-1] < min(temp_df.Close.iloc[-1], temp_df.Open.iloc[-1])
